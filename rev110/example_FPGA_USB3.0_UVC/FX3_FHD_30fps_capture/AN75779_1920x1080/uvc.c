...
uint8_t glProbeCtrl[CY_FX_UVC_MAX_PROBE_SETTING] = {
    ...
    //    							//1280*720*2*2.25 =  4147200 = 0x003F4800
    //    0x00, 0x90, 0x7E, 0x00,     /* Max video frame size in bytes */
    //    							//2560*720*2*2.25 =  8294400 = 0x007E9000
        0x00, 0x20, 0xFD, 0x00,     /* Max video frame size in bytes */
        							//5120*720*2*2.25 = 16588800 = 0x00FD2000
        							//위엣것으로하면 AMCAP에서 안나옴
...
CyFxUVCApplnInit (void)
{
    ...
    //example_FPGA_USB3.0_UVC comment out//SensorReset ();
    //example_FPGA_USB3.0_UVC comment out//SensorInit ();
        ...
        case CY_FX_UVC_PU_BRIGHTNESS_CONTROL:
            switch (bRequest)
            {
                ...
                case CY_FX_USB_UVC_GET_CUR_REQ: /* Current brightness value. */
                	//example_FPGA_USB3.0_UVC comment out//glEp0Buffer[0] = GetBrightness ();
                    CyU3PUsbSendEP0Data (2, (uint8_t *)glEp0Buffer);
                    break;
                ...
                case CY_FX_USB_UVC_SET_CUR_REQ: /* Update brightness value. */
                    ...
                    if (apiRetStatus == CY_U3P_SUCCESS)
                    {
                        ...
                        {
                        	//example_FPGA_USB3.0_UVC comment out//SensorSetBrightness (glEp0Buffer[0]);
                        }
                    }
                    break;
                default:
            ...
            switch (bRequest)
            {
                ...
                case CY_FX_USB_UVC_SET_CUR_REQ:
                    ...
                    if (apiRetStatus == CY_U3P_SUCCESS)
                    {
                        if (usbSpeed == CY_U3P_SUPER_SPEED)
                        {
                        	//example_FPGA_USB3.0_UVC comment out//SensorScaling_HD720p_30fps ();
#ifdef FRAME_TIMER_ENABLE
                        ...
                        {
                        	//example_FPGA_USB3.0_UVC comment out//SensorScaling_VGA ();
#ifdef FRAME_TIMER_ENABLE
